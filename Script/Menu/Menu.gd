...
@onready var ready_screen = $Background/CustomiseGameScreen
...
func _ready():
	$Background/CustomiseGameScreen.connect("ready_pressed", Callable(self, "_on_ready_screen_ready_pressed"))

func _on_play_screen_hidden(info):
	print("info in _on_match_screen_hidden:", info)
	$Background/CustomiseGameScreen.show()
	#Permet d'appeller une fonction d'un script depuis un autre script
	$Background/CustomiseGameScreen.callv("_show_screen", [info])
