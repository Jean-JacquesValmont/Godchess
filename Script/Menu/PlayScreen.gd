...
@onready var join_match_id := $CustomMatch/IDMatchText
...
#	$Panel/FindMatch/SearchButton.connect("pressed", Callable(self, "_on_match_button_pressed").bind(OnlineMatch.MatchMode.MATCHMAKER))
	$CustomMatch/ButtonCreate.connect("pressed", Callable(self, "_on_match_button_pressed").bind(OnlineMatch.MatchMode.CREATE))
	$CustomMatch/ButtonJoin.connect("pressed", Callable(self, "_on_match_button_pressed").bind(OnlineMatch.MatchMode.JOIN))
...
func _on_button_create_button_down():
