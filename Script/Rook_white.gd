...
#		print(player_in_game.keys()[0])
	...
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT and event.pressed and player_in_game != null:
		print("player_in_game.keys()[0]:", player_in_game.keys()[0])
		...
		if player_in_game.keys()[1] == OnlineMatch._nakama_multiplayer_bridge.multiplayer_peer._self_id:
...
func _on_Area2D_area_entered(area):
	...
	get_node("/root/Main/Game/background/plateau/" + area.get_parent().get_name()).queue_free()
	print("piece pris: ","/root/Main/Game/background/plateau/" + area.get_parent().get_name())
